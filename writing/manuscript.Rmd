...
Census data and geometries must be associated to one another, possibly across large spatial and temporal scales and so that they are compatible across distinct efforts.

This entails:

1. Administrative/territorial units must be matched across different languages.
2. Likewise, particular key variables (species, commodities, ...) might have different values in different languages or use different ontologies and they must also be matched.
3. To make variables comparable across different efforts, there must be a standard for naming variables (*well... then this might be a topic that we have identified as crucial, but that censusTools doesn't solve, as there is no commonly defined ontology available yet. Hence we should perhaps make a call for such efforts at the end of the discussion!?*).
4. Moreover, we believe that full transparency and reproducibility are crucial so that changes that may become necessary based on a different view of a set of problems or different demands to the data can be compared meaningfully with previous efforts.

A generalised framework for managing census data that would be related to spatial information, irrespective of the covered variables, was devised and shall be presented in this paper. As example data-set we use ...

*example use-case on a dataset that is already publicly available, this could be a combination of the data (from the US census) that can be obtained with one of the already existing census packages (see below) and the relatively open data from brazil, which need to be downloaded with a custom workflow (i.e. wget).*

# Description of `censusTools`

This section describes briefly what `censusTools` does and showcases the example. In the following subsections the technical details are outlined.
...
Moreover, it can't be expected that any of the data come in the same format, neither with regard to the file type nor with regard to how the data are arranged inside those files.
...
setPath(root = "/home/se87kuhe/my_science/r-dev/censusTools/writing/newProject")
```
...
An index contains at least the values of the target variable and unique IDs for each value.
Additionally arbitrary data that describe the values further may be provided (e.g. details on collecting/sampling the data, the scientific name, a description or ...).
A translation table contains at least the values that should be translated, the values of the translated target variable (same terms as in the index) and a statement that describes the source of the translations.

```{r}
# provide a table that is primarily an index table, i.e. it contains terms and a related ID
read_csv(file = "fao_commodities.csv") %>% 
  setIndex(output = "commodities", pid = "faoID", target = "simpleName")

# this creates the tables 'id_commodities' and 'tt_commodities'
read_csv(file = "newProject/id_commodities.csv", col_types = "iclciccc")
read_csv(file = "newProject/tt_commodities.csv", col_types = "ccc")

# provide a table that is primarily a translation table
# SE: must still check whether this works
```
...
The resulting inventory tables document which geometry and census files are part of the data-base, which ID they have, where and when the data have been stored.
...
Both, terms from different languages and terms from different ontologies need to be translated into one common language, this is covered by a translation table.
In those cases so-called gazetteers can help to find sort of a standard.
For instance, the GADM data-set comes with alternative names for numerous territorial units, which can be used for alternative matching.
The GBIF data-set contains the backbone taxonomy [REF], which could be used for this purpose when it comes to species (*SE: additional/other/better suggestions for widely known tables of that kind?*).
However, since it can't be guaranteed that all data providers use only names that have been covered by the included translation tables, the function `translateTerms()` allows that new mappings between so far not recorded terms and target terms are made.
...
The translation section (upper part) of the table contains all `origin <-> target` mappings.
This section is populated when using `translateTerms()`.
Alternatively one could insert an already existing translation table.

| origin | target | notes |
| :- | :- | :- |
| Beans, dry | beans dry | translateTerms() on 2019-03-25|
| Poroto seco | beans dry | translateTerms() on 2019-03-19|
| ... | ... |  |
|  | beans dry | original |
|  | broad beans dry | original |
|  | peas dry | original |


The look-up section (lower part) contains the target terms to which new, unmapped terms should be translated.
In this section one would insert the terms of a standardised ontology.
When encountering new terms, those target terms are used for fuzzy matching, where a list of the three most highly matched terms based on the Levenshtein distance [REF] is returned, from which the user has to chose.
The column `origin` remains empty and the column `source` must contain the value *original* for the function `translateTerms()` to recognise the distinction between translation and look-up section.
