...
test_that("recognise several vertical clusters of otherwise tidy data", {
  ...
  input1 <- read_csv(paste0(system.file("test_datasets", package="censusTools"), "/table1.csv"),
...
test_that("recognise several horizontal clusters of otherwise tidy data", {
  ...
  input2 <- read_csv(paste0(system.file("test_datasets", package="censusTools"), "/table2.csv"),
                    col_names = FALSE) %>%
    record(what = "algorithm", name = "algo") %>%
    reorganise()
...
test_that("rename variables, in wide table", {
  ...
  input3 <- read_csv(paste0(system.file("test_datasets", package="censusTools"), "/table3.csv"),
                    col_names = FALSE) %>%
    record(what = "algorithm", name = "algo") %>%
    reorganise()
  ...
  algo <<- list(clusters = list(top = NULL, left = NULL, width = NULL, height = NULL,
               ...
               variables = list(territories =
                                ...
                                production =
                                  list(type = "values", unit = "t", factor = 1,
                                       row = NULL, col = 5, rel = FALSE,
                                       id = "dimension", value = "production")))

  input4 <- read_csv(paste0(system.file("test_datasets", package="censusTools"), "/table4.csv"),
                    col_names = FALSE) %>%
    record(what = "algorithm", name = "algo") %>%
    reorganise()
...
test_that("bring wide identifying variable into long form", {
  ...
  input5 <- read_csv(paste0(system.file("test_datasets", package="censusTools"), "/table5.csv"),
                    col_names = FALSE) %>%
    record(what = "algorithm", name = "algo") %>%
    reorganise()
...
test_that("bring wide identifying variable into long form and spread long table", {
  ...
  input6 <- read_csv(paste0(system.file("test_datasets", package="censusTools"), "/table6.csv"),
                    col_names = FALSE) %>%
    record(what = "algorithm", name = "algo") %>%
    reorganise()
...
test_that("bring several wide identifying variables into long form", {
  ...
  input7 <- read_csv(paste0(system.file("test_datasets", package="censusTools"), "/table7.csv"),
                    col_names = FALSE) %>%
    record(what = "algorithm", name = "algo") %>%
    reorganise()
