...
test_that("geometries can be normalised", {
  ...
  final <- st_read(dsn = paste0(getOption("adb_path"), "/adb_geometries/stage3/a_nation.gpkg"), layer = "level_1", quiet = TRUE)
  expect_class(x = final, classes = c("sf"))
  expect_data_frame(x = final, nrows = 1, ncols = 5)
  expect_names(x = names(final), identical.to = c("ahName", "ahID", "level", "geoID", "geom"))

  final <- st_read(dsn = paste0(getOption("adb_path"), "/adb_geometries/stage3/a_nation.gpkg"), layer = "level_2", quiet = TRUE)
  expect_class(x = final, classes = c("sf"))
  expect_data_frame(x = final, nrows = 4, ncols = 5)
  expect_names(x = names(final), identical.to = c("ahName", "ahID", "level", "geoID", "geom"))

  final <- st_read(dsn = paste0(getOption("adb_path"), "/adb_geometries/stage3/a_nation.gpkg"), layer = "level_3", quiet = TRUE)
  expect_class(x = final, classes = c("sf"))
